 ...
 * 완료 후 saleson.security.seed-users 플래그를 false로 되돌릴 것.
 */
@Slf4j
@Component
...
public class UserSeedRunner implements CommandLineRunner {
    ...
    @Value("${saleson.security.seed-users:false}")
    private boolean enabled;

    @Value("${saleson.security.seed-users-admin-pw:sjinc21}")
    ...
    public void run(String... args) throws Exception {
        ...
        log.warn("[SEED-USERS] tb_user emp_no/email/pw 업데이트 완료 — updated={}, notFound={}, admin pw set. " +
                "application.yml 의 saleson.security.seed-users 를 false 로 되돌릴 것.", updated, notFound);
    }
}
