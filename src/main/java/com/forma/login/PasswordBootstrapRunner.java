...
/**
 * 비밀번호 일괄 초기화 1회성 러너.
 * application.yml에서 forma.security.reset-all-passwords=true 로 켠 뒤 재기동하면
...
public class PasswordBootstrapRunner implements CommandLineRunner {
    ...
    @Value("${forma.security.reset-all-passwords:false}")
    private boolean resetAll;

    @Value("${forma.security.reset-all-passwords-value:1}")
    ...
    public void run(String... args) {
        ...
        log.warn("[SECURITY] 전체 사용자 비밀번호를 '{}' 로 초기화 완료 ({}건). " +
                "운영 전환 시 application.yml 의 forma.security.reset-all-passwords 를 false 로 되돌릴 것.",
                resetValue, count);
    }
}
