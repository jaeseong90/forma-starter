...
- `FORM_DEMO.html` 전면 재작성 (855 → 419 줄): 21개 demo-section 을 위젯 그룹 5개(텍스트·숫자 7 / 선택 8 / 날짜·기간 7 / 범위·평점·색·태그 4 / 팝업·파일·표시 4) + 공통 기능 데모 + 검색폼 + API 레퍼런스로 재구성. 각 그룹 아래 "코드 보기" details 로 elements 배열 노출. setData 로 의미있는 초기값 시드.
- `GRID_DEMO.html` 상단 재구성 (1126 → 1187 줄, API 레퍼런스를 하단 이동 + 표 가독성 개선): 헤더의 plain `<h1>` 유지, 상단 API 레퍼런스 블록을 페이지 하단 `<details class="fd-ref">` 로 이동(생성자 19 / 컬럼 15 / 메서드 25+ / 이벤트 5 / 키보드 단축키). 표에 sticky 헤더, 모노스페이스 시그니처 컬럼, 그룹 헤더 행 적용. 18번 섹션 제목 typo("16. N단 멀티헤더 (4단)" → "18. ...") 수정. 21개 라이브 데모 섹션 모두 보존.
